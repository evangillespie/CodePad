...
void StateMachine::_initialize_system_leds(){
	...
	//Landscape tube motor as output
	pinMode(3, OUTPUT);

	//Landscape tube motor off
	digitalWrite(3, LOW);
	...
	//Power crystals set at brightness 40
	analogWrite(LED_12_PIN, 40);

	//Yellow keypad LEDs set at brightnes 40
	analogWrite(LED_1_PIN, 40);

	// Radar LED starts at brightness 20 
	analogWrite(LED_5_PIN, 20);  
